  ...
  return (
    <>
      <Translate
          ...
          className="fixed z-30 right-4 md:right-8 top-2 md:top-4 bg-blue-500 text-zinc-50 p-2 rounded-full cursor-pointer hover:scale-110 transition-all"
        />
        <div
          className={`fixed z-30 right-4 md:right-8 top-16 md:top-20 bg-zinc-400 ${isTranslateMenuOpen ? 'scale-100' : 'scale-0'} transition-all p-2 rounded-md flex flex-col gap-2`}
        >
          <button
            onClick={() => lang !== 'en' && handleChangeLanguage('en')}
            className={`${lang === 'en' ? 'bg-blue-700' : 'bg-zinc-500'} rounded-md p-2 hover:scale-105 hover:bg-blue-600 transition-all text-zinc-100`}
          >
            English
          </button>
          <button
            onClick={() => lang !== 'pt' && handleChangeLanguage('pt')}
            className={`${lang === 'pt' ? 'bg-blue-700' : 'bg-zinc-500'} rounded-md p-2 hover:scale-105 hover:bg-blue-600 transition-all text-zinc-100`}
          >
            Português
          </button>
        </div>
